    ...
    <div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map(row => (
              <tr key={row.date} className="border-b border-brand-border hover:bg-brand-bg transition-colors">
                <td className="py-3 text-brand-muted text-xs">{formatDate(row.date)}</td>
                <td className="py-3 text-right text-brand-text tabnum font-medium">{row.nav.toFixed(4)}</td>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-1.5 rounded-lg border border-brand-border text-brand-muted hover:text-brand-text hover:border-brand-green disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
              <ChevronLeft size={16} />
            </button>
            <button
